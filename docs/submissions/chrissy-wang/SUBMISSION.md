...
- **Solution:** Stage one is real and running for two cities. Houston/Texas watches 46 feeds plus custom readers for SEC EDGAR filings, Texas WARN layoff notices, TDLR contractor licenses, and HCAD commercial property records. New York City/NY State watches 66 feeds plus NY and NJ WARN notices, EDGAR, DOB building permits, DCWP business licenses, and ACRIS property deeds. It checks every 10 minutes and delivers one digest per city. A source-discovery step now proposes a new city's starting source list instead of my assembling it by hand: asked cold for Houston business, 6 of its 7 proposals matched the config I built by hand over months, and it flags bodies reporters cite that publish nothing subscribable — where hand-searching pays off. Stages two through five — the feedback loop, the reporting brief, drafting, and video — are designed but not built yet. Known limits on what does exist: it runs on my own Mac rather than in the cloud, so it stops when my laptop sleeps; deduplication keys on the raw URL, so the same story on two wires can still reach me twice; and the discovery step is a starting set a reporter still has to extend, not a finished source list.
